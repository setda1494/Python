...
def fetch_and_display_news():
    ...
    # HTTP GET 요청
    params = {
        'q': '경제 OR IT OR AI OR 보안 OR 사이버 OR 해킹',  # 보안 관련 키워드 추가
        'language': 'ko',
        'apiKey': api_key
    }
    ...
    if response.status_code == 200:
        ...
        for article in articles:
            title = article['title']
            link = article['url']
            topic = ('경제' if '경제' in title else
                     'IT' if 'IT' in title else
                     'AI' if 'AI' in title else
                     '보안' if '보안' in title or '사이버' in title or '해킹' in title else None)
