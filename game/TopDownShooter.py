...
# 상태 이상 적 클래스
class StatusEffectEnemy:

    def __init__(self):
        self.image = pygame.Surface((40, 40))
        self.health = 25  # 상태 이상 적의 건강
        self.speed = 2
        self.rect = self.image.get_rect(center=(random.randint(0, WIDTH),
                                                random.randint(0, HEIGHT)))
        self.effect_type = random.choice(['poison', 'slow', 'bleed'])
        self.image.fill(RED)  # 상태 이상 적 색상
...
class SupportEnemy:
    ...
    def __init__(self):
        self.image = pygame.Surface((40, 40))
        self.health = 30  # 보조 적의 건강
        self.speed = 2
        self.rect = self.image.get_rect(center=(random.randint(0, WIDTH),
                                                random.randint(0, HEIGHT)))
        self.image.fill(BLUE)  # 보조 적 색상

    def update(self, player):
        # 플레이어의 공격을 방어하는 로직을 구현할 수 있음
        if self.rect.colliderect(player.rect):
            self.attack(player)

    def attack(self, player):
        print("Support enemy is blocking your attack!")
...
class Stage:
    ...
    def create_enemies(self):
        ...
        if self.stage_number == 1:
            enemies.extend([Enemy('normal') for _ in range(5)])
        elif self.stage_number == 2:
            enemies.extend(
                [Enemy(random.choice(['normal', 'fast'])) for _ in range(7)])
        else:
            enemies.extend([
                Enemy(random.choice(['normal', 'strong'])) for _ in range(10)
            ])
            enemies.extend([StatusEffectEnemy()
                            for _ in range(3)])  # 상태 이상 적 추가
            enemies.extend([SupportEnemy() for _ in range(2)])  # 보조 적 추가
        return enemies
